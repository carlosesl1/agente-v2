...
| R76 | Composition root promovível mantém adapter obrigatório ausente | 3 | 5 | startup passa e primeiro webhook falha | uma factory canônica, `--factory`, graph completo e boot/readiness RED | runtime | aberto |
| R77 | Reply/receipt nasce depois do commit ou duplicate reinvoca Maya | 3 | 5 | crash perde resposta ou replay chama LLM/read/kernel | reply, chunks, transcript e receipt no mesmo boundary commit; duplicate retorna bytes persistidos | boundary/runtime | aberto |
| R78 | Worker stale faz primeiro target commit ou external send após cancel/reconcile | 3 | 5 | target/outcome aparece depois de terminalização | execution lock comum, lookup side-effect-free, clock sob lock e terminalização fail-closed | execution/messaging | aberto |
| R79 | Qualification/cancellation fecha sem drenar admissions, parents ou children | 3 | 5 | `CANCELLED` com row ativa, efeito pendente ou child sem decisão | freeze conjunto run+admission, lead locks, close dependency-aware e receipts bilaterais | release/domain | aberto |
| R80 | RED ou review não está ligado aos mesmos bytes funcionais/evidência | 3 | 5 | output hash sem source tree, tests mudam antes de F ou pareceres em SHAs distintos | U/P/S/R/O, source F + evidence child E e AND gate no mesmo par/package | QA/release | aberto |
| R81 | Source attestation ou release manifest forma ciclo/aceita contexto mutável | 2 | 5 | manifest inclui próprio hash, `COPY` alcança membro não listado ou builder recebe diretório | payload manifest fechado, approval combinado externo, tar canônico e poison tests | release/security | aberto |
| R82 | Abandono/reopen de memory root deixa payload órfão ou reutiliza epoch antigo | 2 | 5 | target terminal com payload, journal pendente sem recovery ou ACK antigo aceito | intent-before-clone, S0–S5/A0–A4, target `ABANDONING`, tombstone/fsync/zero-scan e ACK bilateral | persistence/runtime | aberto |
| R83 | Mixed mode permite mutator legacy atravessar freeze | 3 | 5 | writer com permit antigo comita após snapshot final | `migration-ownership-v1` único, permit revalidado no commit e cutover global quiescente se guard incompleto | migration/runtime | aberto |
