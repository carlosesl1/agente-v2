...
- Exact next gate: freeze the second prompt correction and rerun only the `agency-solo-4ps` real-model hotspot with fake reads in a fresh journal. Run static/canonical tests and the remaining semantic matrix only if that hotspot passes. No push, deploy, restart, canary, OCI build, provider write, outbound message, payment initiation, or promotion is authorized.
