...
- Exact next gate: freeze the post-smoke prompt correction, rerun static and canonical tests on the new SHA, then repeat the corrected smoke in a fresh journal. Only if manual review passes may the five historical conversations run with fake reads. No push, deploy, restart, canary, OCI build, provider write, outbound message, payment initiation, or promotion is authorized.
