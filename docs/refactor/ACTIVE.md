...
- Exact next gate: freeze the third prompt correction and rerun only `hostel-couple-private` with the corrected fake knowledge fixture and `package-solo-4ps-late` with the corrected two-command harness expectation. Run static/canonical tests and final audit only if both pass. No push, deploy, restart, canary, OCI build, provider write, outbound message, payment initiation, or promotion is authorized.
