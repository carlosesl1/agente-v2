...
- Exact NEXT: execute plan Task 2 test-first: dedicated encrypted monotonic SQLite trace store under the ops directory, with writer-only bootstrap, read-only reader, exact event/lead grouping, idempotency, legal transitions, AES-256-GCM full content, bounded deterministic queries and contention tests. Do not instrument runtime, edit deploy files or advance to Task 3 until Task 2 is committed and reviewed.
