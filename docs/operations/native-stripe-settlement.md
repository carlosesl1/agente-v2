...
- Isolated pre-final full suite: **2443 tests and 2958 subtests passed**. The subsequent configured-base/currency correction passed **37 focused tests** and requires a new final full run in the external release evidence. Focused boundary regression: **71 tests and 6 subtests passed**. The earlier broad run identified layer-placement and generated-manifest failures; both were corrected and the final full run supersedes it.
