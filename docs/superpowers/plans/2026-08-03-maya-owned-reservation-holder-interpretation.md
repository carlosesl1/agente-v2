...
- Accepted private-update replies are parent-owned unless the turn reaches a parent-generated summary, even when a proposed provider read is later rejected or filtered to zero; Maya's natural clarification is preserved only when no private fact was accepted.
