...
import v2_host.structured_output as structured_output
...
def test_structured_output_exposes_only_maya_v8_contract() -> None:
    assert not hasattr(structured_output, "GROUNDING_REVIEW_JSON_SCHEMA")
    assert not hasattr(structured_output, "grounding_review_request_overrides")
    assert structured_output.__all__ == ["maya_v8_request_overrides"]
