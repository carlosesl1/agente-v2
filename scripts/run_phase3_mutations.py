...
MUTANTS = (
    ...
    Mutant(
        name="remove_cloudbeds_property_from_offer_target",
        path="reservation_lookup/cloudbeds.py",
        old='''        provider_ref = (\n            f"cloudbeds.property.{request.property_id}."\n            f"room.{room_id}.rate.{rate_plan_id}"\n        )\n''',
        new='''        provider_ref = f"cloudbeds.property.synthetic.room.{room_id}.rate.{rate_plan_id}"\n''',
        test="tests.test_phase3_cloudbeds_adapter.CloudbedsBoundaryTests.test_property_id_is_part_of_executable_offer_identity",
    ),
    Mutant(
        name="unbind_responses_from_request_fingerprints",
        path="reservation_lookup/identity.py",
        old='''    return tuple(sorted(zip(requests, responses, strict=True)))\n''',
        new='''    return tuple(zip(sorted(requests), sorted(responses), strict=True))\n''',
        test="tests.test_phase3_lookup_types.OpaqueIdentityTests.test_snapshot_hash_binds_each_response_to_its_request",
    ),
    Mutant(
        name="accept_rebound_lookup_id",
        path="reservation_lookup/types.py",
        old='''        if self.evidence.lookup_id != expected_lookup_id:\n            raise ValueError("evidence lookup_id is not canonical")\n''',
        new="",
        test="tests.test_phase3_lookup_types.LookupResultContractTests.test_result_rejects_rebound_noncanonical_lookup_id",
    ),
    Mutant(
        name="accept_zero_total_positive_offer",
        path="reservation_lookup/types.py",
        old='''            if offer.total.amount <= 0:\n                raise ValueError("positive offers must have positive total")\n''',
        new="",
        test="tests.test_phase3_lookup_types.LookupResultContractTests.test_positive_result_rejects_zero_total",
    ),
    Mutant(
        name="make_expiration_boundary_inclusive",
        path="reservation_domain/types.py",
        old='''        return self.observed_at <= instant < self.expires_at\n''',
        new='''        return self.observed_at <= instant <= self.expires_at\n''',
        test="tests.test_phase3_selection.ExactOfferSelectionTests.test_negative_uncertain_and_expired_lookup_fail_closed",
    ),
    Mutant(
        name="reuse_same_internal_target_in_cross_target_property",
        path="reservation_lookup/properties.py",
        old='''            target_variant=1,\n''',
        new='''            target_variant=0,\n''',
        test="tests.test_phase3_properties.Phase3PropertyTests.test_property_runner_exercises_both_authorization_directions",
    ),
    Mutant(
